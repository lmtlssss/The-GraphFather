...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let data = data
        .or_else(|| env::var_os("GRAPHFATHER_DATA").map(PathBuf::from))
        .or_else(|| env::var_os("PLUGIN_DATA").map(PathBuf::from))
        .or_else(|| {
            env::var_os("CODEX_HOME")
                .map(|p| PathBuf::from(p).join("plugins/data/the-graphfather-the-graphfather"))
        })
        .or_else(|| {
            env::var_os("HOME").map(|p| {
                PathBuf::from(p).join(".codex/plugins/data/the-graphfather-the-graphfather")
            })
        })
    ...
    if cmd == "hook" {
        if let Err(e) = hooks::run(&data) {
            eprintln!("the-graphfather hook advisory skipped: {e}");
        }
        return Ok(());
    }
    if cmd == "trust" {
