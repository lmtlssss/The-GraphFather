...
cleanup(){ if [ -n "${tmp:-}" ]; then rm -f "$tmp" "${sum:-}"; fi; rm -f "${regerr:-}"; }
...
register_marketplace() {
 if [ -d "$source" ]; then
  ...
  rm -f "$regerr"
  git ls-remote --exit-code "$source" "refs/tags/$desired_ref" >/dev/null || return 1
  config_backup="$data/.marketplace-config.backup.$$"
  [ -f "$codex_dir/config.toml" ] && { cp "$codex_dir/config.toml" "$config_backup" || return 1; chmod 600 "$config_backup" || return 1; }
  CODEX_HOME="$codex_dir" "$codex" plugin marketplace remove the-graphfather >/dev/null || return 1
...
trust_owned() {
 CODEX_HOME="$codex_dir" CODEX_BIN="$codex" "$old" --data-dir "$data" trust ||
 CODEX_HOME="$codex_dir" CODEX_BIN="$codex" "$old" --data-dir "$data" trust
}
